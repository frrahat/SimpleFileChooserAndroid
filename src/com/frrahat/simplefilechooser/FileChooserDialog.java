...
public class FileChooserDialog extends DialogFragment {
	...
	String[] fileFormats;
	boolean isShownHiddenFolders;
	
	public FileChooserDialog(String[] fileFormats, boolean isShownHiddenFolders) {
		this.fileFormats=fileFormats;
		this.isShownHiddenFolders=isShownHiddenFolders;
	}
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		...
		if (Environment.getExternalStorageState() != null) {//SD card available
			root = new File(Environment.getExternalStorageDirectory(),
					MainActivity.startingFoldername);
			if(!root.exists()){
				root.mkdirs();
			}
		} else {
	...
	private void updateDisplayFileList() {
		...
		if (allFiles != null) {
			for (int i = 0; i < allFiles.length; i++) {
				...
				if (!isShownHiddenFolders && file.getName().startsWith("."))
					continue;
